...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		//print_r($this->model_catalog_product->getProductHowUse($this->request->get['product_id']));

		if (isset($this->request->post['product_howuse'])) {
			$data['product_howuse'] = $this->request->post['product_howuse'];
		} elseif (isset($this->request->get['product_id'])) {
			$data['product_howuse'] = $this->model_catalog_product->getProductHowUse($this->request->get['product_id']);
		} else {
			$data['product_howuse'] = array();
		}
		

		$data["howUseCount"] = count($data['product_howuse']);
		...
		if (isset($this->request->post['image_how_to_use'])) {
			$data['image_how_to_use'] = $this->request->post['image_how_to_use'];
		} elseif (!empty($product_info)) {
			$data['image_how_to_use'] = $product_info['image_how_to_use'];
		} else {
			$data['image_how_to_use'] = '';
		}

		$this->load->model('tool/image');

		if (isset($this->request->post['image_how_to_use']) && is_file(DIR_IMAGE . $this->request->post['image_how_to_use'])) {
			$data['thumb_how_to_use'] = $this->model_tool_image->resize($this->request->post['image_how_to_use'], 100, 100);
		} elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_info['image_how_to_use'])) {
			$data['thumb_how_to_use'] = $this->model_tool_image->resize($product_info['image_how_to_use'], 100, 100);
		} else {
			$data['thumb_how_to_use'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		...
		$data['button_save'] = $this->url->link('category/product/uploadProductVideo', 'user_token=' . $this->session->data['user_token'], true);
	...
	public function uploadProductVideo(){
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST'){
            ...
            $this->db->query("UPDATE `oc_product` SET `video`='./public/video/".basename($_FILES['our-file']['name']) . "' WHERE `product_id`='" . $this->request->get["product_id"] ."';");
            return "test";
		}
	...
	public function deletevideo(){
        $this->db->query("UPDATE `oc_product` SET `video`='' WHERE `product_id`='" . $this->request->get["product_id"] ."';");
