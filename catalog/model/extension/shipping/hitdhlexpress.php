...
class ModelExtensionShippinghitdhlexpress extends Model {
	function getQuote($address) {
		...
		//print_r("ModelExtensionShippinghitdhlexpress 1");
		...
		if ($status) {
			...
			//print_r("ModelExtensionShippinghitdhlexpress 2");
			...
			foreach($products as $sing_product)
			{
				if(isset($sing_product['shipping']) && $sing_product['shipping'] == 1)
				{
					...
					// echo '<pre>'; print_r($sing_product);die();
					$get_product[] = $sing_product;
				}
			}
			
			
			//print_r("ModelExtensionShippinghitdhlexpress 3");
			...
			$dutiable_content = "<Dutiable><DeclaredCurrency>{$selected_currency}</DeclaredCurrency><DeclaredValue>{$total_value}</DeclaredValue></Dutiable>";
			...
			//print_r("ModelExtensionShippinghitdhlexpress 4");
			...
			$xml .= '			<PaymentCountryCode>US</PaymentCountryCode>';
			...
			$curl = curl_init();
			curl_setopt($curl, CURLOPT_POSTFIELDS, utf8_decode($xml));
			...
			//print_r("ModelExtensionShippinghitdhlexpress 5");
			...
			// print_r("-=-=-=-=-=-=-=-=-=-=-=-=-");
			// print_r("<br>");
			//print_r(htmlspecialchars(curl_exec($curl)));
			// print_r("<br>");
			// print_r("-=-=-=-=-=-=-=-=-=-=-=-=-");
			// print_r("<br>");
			// print_r("<br>");
			...
			if ($result && !empty($result->GetQuoteResponse->BkgDetails->QtdShp)) {
				...
				foreach ($result->GetQuoteResponse->BkgDetails->QtdShp as $quote) {
					...
					if(isset($quote->DeliveryDate) && isset($quote->DeliveryTime)){
						...
						$formated_date = DateTime::createFromFormat('Y-m-d', (string)$quote->DeliveryDate);
